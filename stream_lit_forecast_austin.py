...
def ECMWF_forecast(lat = 30.2672, lon = -97.7431):
    data = xr.open_dataset('https://raw.githubusercontent.com/nvnsudharsan/streamlit_forecast_austin/blob/main/forecast_ecmwf_may_.nc', engine='netcdf4')
    t2m = data.t2m
    t2m = t2m - 273.15
    t2m = (t2m*(9/5))+32
...
def ECMWF_anom(month):
    anom_fc         = xr.open_dataset('https://raw.githubusercontent.com/nvnsudharsan/streamlit_forecast_austin/blob/main/type_fcmean.nc',engine='netcdf4')
    anom_fc         = anom_fc.t2a
